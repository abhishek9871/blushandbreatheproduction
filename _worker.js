...
Return JSON:
{
  ...
  "alternatives": ["Option 1", "Option 2"]
    ...
    // GET /api/banned - List all banned substances
    if (path === '/api/banned' && request.method === 'GET') {
      try {
        if (env.SUBSTANCE_CACHE) {
          const index = await env.SUBSTANCE_CACHE.get('substance:index:banned', 'json');
          if (index && Array.isArray(index)) {
            const substances = [];
            for (const slug of index) {
              const item = await env.SUBSTANCE_CACHE.get(`substance:banned:${slug}`, 'json');
              if (item) substances.push(item);
            }
            return new Response(JSON.stringify({ success: true, data: substances }), {
        ...
        console.error('Error fetching banned substances:', error);
        return new Response(JSON.stringify({ success: false, error: 'Failed to fetch banned substances' }), {
          status: 500,
          headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' }
        });
      }
    ...
    // GET /api/banned/:slug - Get specific banned substance
    const bannedMatch = path.match(/^\/api\/banned\/([^\/]+)$/);
    if (bannedMatch && request.method === 'GET') {
      try {
        const slug = decodeURIComponent(bannedMatch[1]);
        if (env.SUBSTANCE_CACHE) {
          const data = await env.SUBSTANCE_CACHE.get(`substance:banned:${slug}`, 'json');
          if (data) {
            return new Response(JSON.stringify({ success: true, data }), {
              headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*', 'Cache-Control': 'public, max-age=3600' }
            });
        ...
        return new Response(JSON.stringify({ success: false, error: 'Substance not found' }), {
          status: 404,
          headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' }
        });
      } catch (error) {
        console.error('Error fetching banned substance:', error);
        return new Response(JSON.stringify({ success: false, error: 'Failed to fetch substance' }), {
          status: 500,
          headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' }
        });
      }
    ...
    // GET /api/banned/:slug/alternatives - Get legal alternatives for a banned substance
    const altMatch = path.match(/^\/api\/banned\/([^\/]+)\/alternatives$/);
    if (altMatch && request.method === 'GET') {
      try {
        const slug = decodeURIComponent(altMatch[1]);
        if (env.SUBSTANCE_CACHE) {
          const banned = await env.SUBSTANCE_CACHE.get(`substance:banned:${slug}`, 'json');
          if (banned && banned.legalAlternatives) {
            const alternatives = [];
            for (const altSlug of banned.legalAlternatives) {
              const alt = await env.SUBSTANCE_CACHE.get(`substance:supplement:${altSlug}`, 'json');
              if (alt) alternatives.push(alt);
            }
            return new Response(JSON.stringify({ success: true, data: alternatives, bannedSubstance: slug }), {
              headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*', 'Cache-Control': 'public, max-age=3600' }
            });
          }
        }
        return new Response(JSON.stringify({ success: true, data: [], bannedSubstance: slug }), {
          headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' }
        });
      } catch (error) {
        console.error('Error fetching alternatives:', error);
        return new Response(JSON.stringify({ success: false, error: 'Failed to fetch alternatives' }), {
          status: 500,
          headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' }
        });
      }
    ...
    // GET /api/supplement - List all supplements
    if (path === '/api/supplement' && request.method === 'GET') {
      try {
        if (env.SUBSTANCE_CACHE) {
          const index = await env.SUBSTANCE_CACHE.get('substance:index:supplements', 'json');
          if (index && Array.isArray(index)) {
            const supplements = [];
            for (const slug of index) {
              const item = await env.SUBSTANCE_CACHE.get(`substance:supplement:${slug}`, 'json');
              if (item) supplements.push(item);
            }
            return new Response(JSON.stringify({ success: true, data: supplements }), {
              headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*', 'Cache-Control': 'public, max-age=3600' }
            });
          }
        }
        ...
        return new Response(JSON.stringify({ success: false, error: 'Failed to fetch supplements' }), {
          status: 500,
          headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' }
        });
      }
    ...
    // GET /api/supplement/:slug - Get specific supplement
    const suppMatch = path.match(/^\/api\/supplement\/([^\/]+)$/);
    if (suppMatch && request.method === 'GET') {
      try {
        const slug = decodeURIComponent(suppMatch[1]);
        if (env.SUBSTANCE_CACHE) {
          const data = await env.SUBSTANCE_CACHE.get(`substance:supplement:${slug}`, 'json');
          if (data) {
            return new Response(JSON.stringify({ success: true, data }), {
              headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*', 'Cache-Control': 'public, max-age=3600' }
            });
        ...
        return new Response(JSON.stringify({ success: false, error: 'Supplement not found' }), {
          status: 404,
          headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' }
        });
      } catch (error) {
        console.error('Error fetching supplement:', error);
        return new Response(JSON.stringify({ success: false, error: 'Failed to fetch supplement' }), {
          status: 500,
          headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' }
        });
      }
    ...
    // GET /api/affiliate/:slug - Get affiliate products for a supplement
    const affiliateMatch = path.match(/^\/api\/affiliate\/([^\/]+)$/);
    if (affiliateMatch && request.method === 'GET') {
      try {
        const slug = decodeURIComponent(affiliateMatch[1]);
        if (env.SUBSTANCE_CACHE) {
          const products = await env.SUBSTANCE_CACHE.get(`substance:affiliate:${slug}`, 'json');
          return new Response(JSON.stringify({ success: true, data: products || [], ingredient: slug }), {
            headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*', 'Cache-Control': 'public, max-age=3600' }
          });
        }
        return new Response(JSON.stringify({ success: true, data: [], ingredient: slug }), {
          headers: { 'Content-Type': 'application/json', 'Access-Control-Allow-Origin': '*' }
        });
      } catch (error) {
