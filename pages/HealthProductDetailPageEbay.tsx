...
const HealthProductDetailPageEbay: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const fetchProduct = async () => {
            ...
            try {
                const data = await getHealthProductDetail(itemId);
                setProduct(data);
            } catch (err) {
                setError(err instanceof Error ? err.message : 'Failed to load product');
                console.error('Health product detail error:', err);
            } finally {
                setLoading(false);
            }
        };
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            ...
            <nav className="flex items-center gap-2 text-sm mb-6">
                <Link to="/health-store" className="text-[#2C7A7B] hover:underline">Health Store</Link>
...
export default HealthProductDetailPageEbay;
