...
const HealthStorePageEbay: React.FC = () => {
    ...
    const getBenefitTag = (cat: typeof categories[number]): string => {
        const benefitMap: Record<typeof categories[number], string> = {
            all: 'FOR HEALTH',
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 px-4">
                    {loading ? (
                        ...
                        products.map((product) => (
                            <Link
                                key={product.id}
                                to={`/health-store/product/${encodeURIComponent(product.id)}`}
...
export default HealthStorePageEbay;
