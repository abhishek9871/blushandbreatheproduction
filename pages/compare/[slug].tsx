...
function ComparisonHeader({ banned, alternative }: { banned: BannedSubstance; alternative: LegalSupplement }) {
  return (
    <div className="bg-gradient-to-r from-red-100 via-gray-100 to-green-100 dark:from-red-900/20 dark:via-gray-900 dark:to-green-900/20 rounded-xl p-6 mb-8">
      <div className="flex items-center justify-center gap-4 mb-4">
        <span className="material-symbols-outlined text-3xl text-blue-600 dark:text-blue-400">balance</span>
        <h1 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white">Substance Comparison</h1>
      </div>
      <div className="grid md:grid-cols-3 gap-4 items-center">
        {/* Banned Side */}
        <div className="text-center">
          <span className="inline-block px-3 py-1 bg-red-600/20 dark:bg-red-600/30 text-red-600 dark:text-red-400 text-xs font-semibold rounded-full mb-2">
            BANNED / RESTRICTED
          </span>
          <h2 className="text-xl font-bold text-red-600 dark:text-red-400">{banned.name}</h2>
          <p className="text-red-500 dark:text-red-300 text-sm mt-1 font-medium">{banned.category}</p>
        </div>
        
        {/* VS */}
        <div className="text-center">
          <div className="inline-flex items-center justify-center w-12 h-12 bg-gray-200 dark:bg-gray-800 rounded-full border-2 border-gray-300 dark:border-gray-700">
        ...
        {/* Alternative Side */}
        <div className="text-center">
          <span className="inline-block px-3 py-1 bg-green-600/20 dark:bg-green-600/30 text-green-600 dark:text-green-400 text-xs font-semibold rounded-full mb-2">
            LEGAL ALTERNATIVE
          </span>
          <h2 className="text-xl font-bold text-green-600 dark:text-green-400">{alternative.name}</h2>
          <p className="text-green-500 dark:text-green-300 text-sm mt-1 font-medium">{alternative.category}</p>
        </div>
      </div>
    </div>
  );
...
function ComparisonTable({ banned, alternative }: { banned: BannedSubstance; alternative: LegalSupplement }) {
  ...
  return (
    <div className="bg-white dark:bg-gray-900/50 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden mb-8 shadow-sm">
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead className="bg-gray-100 dark:bg-gray-800/50">
            <tr>
              <th className="px-4 py-3 text-left text-sm font-semibold text-gray-700 dark:text-gray-300">Category</th>
              <th className="px-4 py-3 text-left text-sm font-semibold text-red-600 dark:text-red-400">
                {banned.name}
              </th>
              <th className="px-4 py-3 text-left text-sm font-semibold text-green-600 dark:text-green-400">
                {alternative.name}
              </th>
            </tr>
          </thead>
            ...
            {comparison.comparison.map((row, index) => (
              <tr key={index} className="hover:bg-gray-50 dark:hover:bg-gray-800/30">
                ...
                <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">
                  <div className="flex items-start gap-2">
                    {row.advantage === 'alternative' && (
                      <span className="material-symbols-outlined text-base text-red-500 dark:text-red-400 flex-shrink-0">cancel</span>
                    )}
                ...
                <td className="px-4 py-3 text-sm text-gray-700 dark:text-gray-300">
                  <div className="flex items-start gap-2">
                    {row.advantage === 'alternative' && (
                      <span className="material-symbols-outlined text-base text-green-500 dark:text-green-400 flex-shrink-0">check_circle</span>
                    )}
    ...
    <div className="grid md:grid-cols-2 gap-6 mb-8">
      ...
      <div className="bg-red-50 dark:bg-red-900/10 border border-red-200 dark:border-red-900/30 rounded-xl p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-semibold text-red-600 dark:text-red-400">Risks of {banned.name}</h3>
        </div>
        <ul className="space-y-2">
          {banned.healthRisks.slice(0, 3).map((risk, i) => (
            <li key={i} className="flex items-start gap-2 text-sm text-gray-700 dark:text-gray-200">
              <span className="material-symbols-outlined text-base text-red-500 dark:text-red-400 flex-shrink-0">cancel</span>
              <span><strong className="text-red-600 dark:text-red-300">{risk.category}:</strong> {risk.description.slice(0, 100)}...</span>
            </li>
          ))}
          {banned.sideEffects.slice(0, 3).map((effect, i) => (
            <li key={`se-${i}`} className="flex items-start gap-2 text-sm text-gray-800 dark:text-gray-100">
              <span className="w-4 h-4 flex items-center justify-center text-red-500 dark:text-red-400">•</span>
              <span>{effect}</span>
            </li>
          ))}
        </ul>
      ...
      <div className="bg-green-50 dark:bg-green-900/10 border border-green-200 dark:border-green-900/30 rounded-xl p-5">
        <div className="flex items-center gap-2 mb-4">
          <span className="material-symbols-outlined text-xl text-green-500 dark:text-green-400">verified_user</span>
          <h3 className="font-semibold text-green-600 dark:text-green-400">Benefits of {alternative.name}</h3>
        </div>
        <ul className="space-y-2">
...
function RecommendedProducts({ products, alternative }: { products: AffiliateProduct[]; alternative: LegalSupplement }) {
  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-900/50 rounded-xl border border-gray-200 dark:border-gray-800 p-6 mb-8">
      ...
      <div className="grid md:grid-cols-3 gap-4">
        {products.slice(0, 3).map((product) => (
          <div
            key={product.id}
            className="bg-white dark:bg-gray-800/50 rounded-lg p-4 border border-gray-200 dark:border-gray-700 hover:border-green-500/50 transition-colors shadow-sm"
          >
            <div className="flex justify-between items-start mb-2">
              <h4 className="font-medium text-gray-900 dark:text-white text-sm">{product.name}</h4>
              {product.thirdPartyTested && (
                <span className="px-2 py-0.5 bg-green-100 dark:bg-green-600/20 text-green-600 dark:text-green-400 text-xs rounded">
                  Tested
                </span>
              )}
            </div>
            <p className="text-gray-500 dark:text-gray-400 text-xs mb-2">{product.brand}</p>
            <div className="flex items-center gap-2 mb-3">
              <span className="text-yellow-500 dark:text-yellow-400 text-sm">★ {product.rating}</span>
              <span className="text-gray-400 dark:text-gray-500 text-xs">({product.reviewCount} reviews)</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-green-600 dark:text-green-400 font-semibold">${product.price}</span>
              <a
  ...
  return (
    <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800/50 rounded-xl p-5">
      <div className="flex items-start gap-3">
        <span className="material-symbols-outlined text-xl text-blue-500 dark:text-blue-400 flex-shrink-0">info</span>
        <div>
          <h4 className="font-semibold text-blue-700 dark:text-blue-300 mb-2">Important Disclaimer</h4>
  ...
  if (router.isFallback) {
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex items-center justify-center">
        <div className="animate-pulse text-gray-600 dark:text-gray-400">Loading comparison...</div>
      </div>
    );
  }
      ...
      <main className="min-h-screen bg-gray-50 dark:bg-gray-950">
        <div className="max-w-5xl mx-auto px-4 py-8">
          ...
          <div className="grid md:grid-cols-2 gap-4 mb-8">
            <Link
              href={`/banned/${banned.slug}`}
              className="flex items-center justify-center gap-2 px-4 py-3 bg-red-100 dark:bg-red-900/20 border border-red-200 dark:border-red-900/50 text-red-600 dark:text-red-400 rounded-lg hover:bg-red-200 dark:hover:bg-red-900/30 transition-colors"
            >
              Learn more about {banned.name}
            </Link>
            <Link
              href={`/supplement/${alternative.slug}`}
              className="flex items-center justify-center gap-2 px-4 py-3 bg-green-100 dark:bg-green-900/20 border border-green-200 dark:border-green-900/50 text-green-600 dark:text-green-400 rounded-lg hover:bg-green-200 dark:hover:bg-green-900/30 transition-colors"
            >
